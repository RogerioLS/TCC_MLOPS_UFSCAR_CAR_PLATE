"""Módulo para converter rótulos de coordenadas de canto para o formato YOLO."""
...
def convert_to_yolo_format(corners: str, img_width: int, img_height: int) -> str:
    """
    Converta coordenadas de canto para o formato YOLO.

    Args:
        corners (str): Coordenadas dos cantos no formato "x1,y1,x2,y2,x3,y3,x4,y4".
        img_width (int): Largura da imagem.
        img_height (int): Altura da imagem.

    Returns:
        str: Coordenadas no formato YOLO.
    """
    # corner coordinates
...
def read_input_file(file_path: str) -> tuple:
    """
    Leia o arquivo de entrada e extraia as informações da placa do carro, layout e cantos.

    Args:
        file_path (str): Caminho para o arquivo de entrada.

    Returns:
        tuple: Placa do carro, layout e cantos.
    """
...
def process_files(
    input_directory: str, output_directory: str, img_width: int, img_height: int
) -> None:
    """
    Process os arquivos de entrada e converta as coordenadas para o formato YOLO.

    Args:
        input_directory (str): Diretório de entrada contendo os arquivos.
        output_directory (str): Diretório de saída para os arquivos convertidos.
        img_width (int): Largura da imagem.
        img_height (int): Altura da imagem.
...
def validation_bbx_conversion(image_path: str) -> None:
    """
    Validate a conversão do bounding box desenhando-o na imagem.
