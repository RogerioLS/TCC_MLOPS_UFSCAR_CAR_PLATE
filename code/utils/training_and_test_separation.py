"""Módulo para organizar amostras de treinamento e teste."""
...
base_dir = (
    "/content/drive/MyDrive/ColabNotebooks/tcc_mba/treinamento_yolo_placa/training"
)
categories = ["cars-br", "cars-me", "motorcycles-br", "motorcycles-me"]


def quantidade_amostras() -> dict:
    """
    Organiza as amostras por categoria e conta o número de imagens e labels.

    Returns:
        dict: Dicionário com a contagem de imagens e labels por categoria.
    """
    amostras_por_categoria = {
        category: {"imagens": 0, "labels": 0} for category in categories
    }
    ...
    for category in categories:
        imagens = [
            f
            for f in os.listdir(os.path.join(base_dir, category))
            if f.endswith(".jpg")
        ]
        labels = [
            f
...
dest_dir_base = (
    "/content/drive/MyDrive/ColabNotebooks/tcc_mba/treinamento_yolo_placa/training_800"
)
categories = ["cars-br", "cars-me", "motorcycles-br", "motorcycles-me"]
...
def organizar_amostras() -> None:
    """
    Organiza as amostras por categoria e as distribui em pastas de treino, validação e teste.
    ...
    for category in categories:
        ...
        if len(imagens) < quantidades_desejadas["train"]:
            print(
                f"Categoria '{category}' não possui amostras suficientes para treino. Utilizando o máximo disponível ({len(imagens)} amostras)."
            )
            ...
            for i in range(num_samples):
                if i < len(imagens):
                    ...
                    src_label = os.path.join(
                        base_dir, category, imagens[i].replace(".jpg", ".txt")
                    )
                    dest_label = os.path.join(
                        dest_dir_base,
                        tipo,
                        "labels",
                        imagens[i].replace(".jpg", ".txt"),
                    )
                    shutil.copy(src_label, dest_label)

        # Print para verificar o número de amostras copiadas
        print(
            f"Categoria '{category}': {quantidades_desejadas['train']} amostras para treino, {quantidades_desejadas['val']} amostras para validação, {quantidades_desejadas['test']} amostras para teste."
        )
